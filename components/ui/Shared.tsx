...
export const CustomSelect: React.FC<CustomSelectProps> = ({ label, value, onChange, options, placeholder, className }) => {
    ...
    return (
        <div className={`relative ${className || ''}`} ref={wrapperRef}>
            ...
            >
                <span className={!selectedOption ? "text-slate-600" : "text-white truncate font-medium"}>
                ...
                <div className="absolute z-[60] w-full mt-2 left-0 neo-popover max-h-60 overflow-y-auto custom-scrollbar animate-fade-in">
                    {options.map((option) => (
                        <div
                            ...
                            className={`px-4 py-3 cursor-pointer text-base font-medium transition border-b border-white/5 last:border-0 font-rajdhani tracking-wide ${value === option.value ? 'bg-blue-600/20 text-blue-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                        >
                            {option.label}
                        </div>
                    ))}
                    {options.length === 0 && (
                        <div className="px-4 py-3 text-base text-slate-500 italic">Nenhuma opção disponível.</div>
                    )}
                </div>
            )}
        </div>
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] overflow-y-auto">
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
                ...
                >
                    <div className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-blue-500 z-20 rounded-tl-2xl"></div>
                    <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-blue-500 z-20 rounded-tr-2xl"></div>
                    <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-blue-500 z-20 rounded-bl-2xl"></div>
                    <div className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-blue-500 z-20 rounded-br-2xl"></div>

                    <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-[#151a25] relative z-10 rounded-t-2xl">
