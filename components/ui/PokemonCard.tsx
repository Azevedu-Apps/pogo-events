...
export const PokemonCard: React.FC<PokemonCardProps> = ({ name, image, shiny, tier, form, onDelete, onImageClick, className }) => {
  ...
  const displayImage = (isHovered && details?.shinyImage) ? details.shinyImage : (details?.image || image);
  ...
  return (
    <div 
        className={`
            ...
            overflow-hidden cursor-pointer transition-all duration-300 ${glowClass}
